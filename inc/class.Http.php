 ...
 */
 class Http {
    ...
    public static function Get($host,$pos,$port=80,$timeout=10){
        
        if((TIMESTAMP % 2 == 1)){ // 部分请求尝试记录执行时间
                $st=get_microtime(); // 接口执行时间 开始
                $fp = fsockopen($host, $port, $errno, $errstr, $timeout);
                $et = get_microtime(); // 接口执行时间 结束
                $exec_t = spent($et,$st);
                if($exec_t>=2000){
                        logs($host."/".$pos." 等待时间:".$exec_t."毫秒",'time_out');        
                }
        }else{
                $fp = fsockopen($host, $port, $errno, $errstr, $timeout);        
        }
